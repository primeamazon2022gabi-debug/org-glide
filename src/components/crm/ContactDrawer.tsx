...
export function ContactDrawer({ contact, onClose, onUpdate, companies, members }: ContactDrawerProps) {
  const { updateContact, addActivity, activities: allActivities, deals: allDeals, stages: allStages, userId } = useCRMStore();
  ...
  const fetchRelated = useCallback(() => {
    if (!contact) return;
    setActivities(allActivities.filter((a: any) => a.contact_id === contact.id)
      .sort((a: any, b: any) => (b.created_at || "").localeCompare(a.created_at || "")));
    setDeals(allDeals.filter((d: any) => d.contact_id === contact.id));
    setStages([...allStages].sort((a: any, b: any) => (a.order || 0) - (b.order || 0)));
  }, [contact, allActivities, allDeals, allStages]);
  ...
  const handleSave = () => {
    ...
    try {
      updateContact(contact.id, {
        ...
        phone: form.phone, title: form.title, status: form.status,
  ...
  const addActivityHandler = () => {
    ...
    try {
      addActivity({
        ...
        title: activityForm.title, body: activityForm.body, user_id: userId,
      });
        ...
        <Tabs defaultValue="overview" className="p-4">
          ...
          <TabsContent value="activities" className="mt-4 space-y-4">
            <div className="space-y-2">
              ...
              <Button size="sm" onClick={addActivityHandler} disabled={!activityForm.title}>Adicionar</Button>
            </div>
            <div className="space-y-2">
              {activities.map((a) => {
