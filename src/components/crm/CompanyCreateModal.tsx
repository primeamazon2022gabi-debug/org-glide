...
export function CompanyCreateModal({ open, onOpenChange, onCreated }: CompanyCreateModalProps) {
  const { addCompany, updateContact, contacts: storeContacts, userId } = useCRMStore();
  ...
  useEffect(() => {
    if (!open) return;
    setContacts(storeContacts.map((c: any) => ({
  ...
  const handleCreate = () => {
    ...
    try {
      ...
      selectedContactIds.forEach((cid) => updateContact(cid, { company_id: created.id }));
      onOpenChange(false);
