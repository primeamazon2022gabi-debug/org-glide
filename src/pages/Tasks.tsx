...
export default function Tasks() {
  ...
  const tasks = useMemo(() => store.activities.filter((a: any) => a.type === "task") as Activity[], [store.activities]);
  ...
  const toggleComplete = (task: Activity) => {
    ...
    store.updateActivity(task.id, { completed_at });
  };
  ...
  const handleSave = () => {
    ...
    if (editTask) {
      store.updateActivity(editTask.id, payload);
