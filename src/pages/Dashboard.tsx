...
import { FinancialDashboardWidgets } from "@/components/crm/FinancialDashboardWidgets";
...
export default function Dashboard() {
  ...
  const deals = store.deals as Deal[];
  const stages = store.stages as Stage[];
  const activities = store.activities as ActivityRow[];
  ...
  const handleSeedDemo = async () => {
    setSeeding(true);
    try {
      const { data, error } = await supabase.rpc("seed_demo_financial_data_auto" as any);
      if (error) throw error;
      const r = (data ?? {}) as { companies?: number; contacts?: number; deals?: number };
      toast({
        title: "Dados fictícios carregados!",
        description: `${r.companies ?? 0} empresas, ${r.contacts ?? 0} contatos e ${r.deals ?? 0} negócios criados.`,
      });
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button
            ...
            disabled={seeding}
          >
            <Sparkles className={`h-3.5 w-3.5 ${seeding ? "animate-pulse" : ""}`} />
