...
export default function Login() {
  ...
  useEffect(() => {
    ...
    // Auto-login com conta demo
    const autoLogin = async () => {
      const demoEmail = "demo@flowcrm.app";
      const demoPassword = "FlowCRM#Demo2026!";
      setLoading(true);
      // Tenta login
      const { error } = await supabase.auth.signInWithPassword({ email: demoEmail, password: demoPassword });
      if (error) {
        // Se não existe, cria a conta demo
        ...
        if (signUpError) {
          console.error("Demo auto-login failed:", signUpError.message);
        }
      }
      setLoading(false);
    };
