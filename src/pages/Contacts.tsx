...
export default function Contacts() {
  ...
  const lastActivityMap = useMemo(() => {
    ...
    store.activities.forEach((a: any) => {
  ...
  const handleOwnerChange = async (contactId: string, newOwnerId: string | null) => {
    store.updateContact(contactId, { owner_id: newOwnerId });
  ...
  const batchDelete = () => {
    ...
    ids.forEach((id) => store.deleteContact(id));
  ...
  const batchChangeStatus = (status: ContactStatus) => {
    const ids = Array.from(selectedContacts);
    ids.forEach((id) => store.updateContact(id, { status }));
    setSelectedContacts(new Set());
